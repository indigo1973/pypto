...
- **Structural Properties**: TypeChecked, BreakContinueValid, NoRedundantBlocks, UseAfterDef, OutParamNotShadowed, NoNestedInCore, InOutUseValid, and PipelineLoopValid are verified at pipeline start by `PassPipeline` and before/after each pass by `VerificationInstrument`
...
| -------- | ------- | ----------- |
| `GetStructuralProperties()` | `{TypeChecked, BreakContinueValid, NoRedundantBlocks, UseAfterDef, OutParamNotShadowed, NoNestedInCore, InOutUseValid, PipelineLoopValid}` | Invariants verified at pipeline start and before/after each pass |
