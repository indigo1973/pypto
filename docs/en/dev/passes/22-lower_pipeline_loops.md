...
**Pipeline position**: After [`NormalizeReturnOrder`](21-normalize_return_order.md), before `CanonicalizeIOOrder` and `InitMemRef` (slot 20.5). Late enough that all tile-structural decisions are made; early enough that `CanonicalizeIOOrder` / `InitMemRef` / `MemoryReuse` see distinct tile vars per clone.
...
- [`CanonicalizeIOOrder`](23-canonicalize_io_order.md) — the IO-order canonicalization pass that runs next, scoped to pipeline bodies
